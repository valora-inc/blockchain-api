...
import { Contracts, getContractAddressesSync } from "../utils";
...
export function popWhich(tokenTransfers: BlockscoutTokenTransfer[], predicate: TransferFilter): BlockscoutTokenTransfer | undefined {
    const index = findLastIndex(tokenTransfers, predicate)
    return index > -1 ? tokenTransfers.splice(index, 1)[0] : undefined
}

export function getWhich(tokenTransfers: BlockscoutTokenTransfer[], predicate: TransferFilter): BlockscoutTokenTransfer | undefined {
...
function findLastIndex<T>(
  ...
  export function containsMintedTokenTransfer(tokenTransfers: BlockscoutTokenTransfer[]): boolean {
    return containsTransferFrom(tokenTransfers, MINTED_TOKEN_ADDRESS)
  }

  export function containsBurnedTokenTransfer(tokenTransfers: BlockscoutTokenTransfer[]): boolean {
    return containsTransferTo(tokenTransfers, MINTED_TOKEN_ADDRESS)
  }

  export function containsTransferFrom(tokenTransfers: BlockscoutTokenTransfer[], senderAddress: Contracts | string): boolean {
    const contractAddress = getContractAddressesSync()[senderAddress as Contracts]
    const sender = contractAddress ? contractAddress : senderAddress

    return getTransferFrom(tokenTransfers, sender) !== undefined
  }

  export function containsTransferTo(tokenTransfers: BlockscoutTokenTransfer[], recipientAddress: Contracts | string): boolean {
    const contractAddress =
      getContractAddressesSync()[recipientAddress as Contracts]
    const recipient = contractAddress ? contractAddress : recipientAddress

    return getTransferTo(tokenTransfers, recipient) !== undefined
  }

  export function getFaucetTransfer(tokenTransfers: BlockscoutTokenTransfer[]): BlockscoutTokenTransfer | undefined {
    return getTransferFrom(tokenTransfers, FAUCET_ADDRESS)
  }

  export function getMintedTokenTransfer(tokenTransfers: BlockscoutTokenTransfer[]): BlockscoutTokenTransfer | undefined {
    return getTransferFrom(tokenTransfers, MINTED_TOKEN_ADDRESS)
  }

  export function getBurnedTokenTransfer(tokenTransfers: BlockscoutTokenTransfer[]): BlockscoutTokenTransfer | undefined {
