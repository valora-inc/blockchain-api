...
import { ExchangeRateAPIInterface } from "../currencyConversion/ExchangeRateAPI";
...
export default class PricesService {
    ...
    db: Knex
    exchangeAPI: ExchangeRateAPIInterface
    cUSDAddress: string

    constructor(db: Knex, exchangeAPI: ExchangeRateAPIInterface) {
        this.db = db
    ...
    /**
     * It returns an estimated prices in given local currency of given token.
     ...
     * @param date 
     */
     async getTokenToLocalCurrencyPrice(token: string, localCurrency: string, date: Date): Promise<BigNumber> {

        try {
            const cUSDPrice = await this.getcUSDPrice(token, date)
            const cUSDToLocalCurrencyPrice = await this.cUSDToLocalCurrency(localCurrency, date)
    ...
    async getcUSDPrice(token: string, date: Date): Promise<BigNumber> {
        ...
        if (!prevPriceRow || !nextPriceRow) {
            throw new Error(`Couldn't find entries in the db to calculate cUSD prices for ${token} at ${date}`)
        }

        return this.estimatePrice(prevPriceRow, nextPriceRow, date)
    }

    private estimatePrice(prevPriceRow: any, nextPriceRow: any, date: any) {
        ...
        // Linnear estimation
        return (prevPrice.times(nextTimestamp - queryTimestamp)
            .plus(nextPrice.times(queryTimestamp - prevTimestamp)))
            .dividedBy(nextTimestamp - prevTimestamp)
    }

    private async cUSDToLocalCurrency(localCurrency: string, date: Date): Promise<BigNumber> {
...
function getcUSDFromConfig(): string {
    ...
    if (!config) {
        throw new Error(`Couldn't obtain exchanges config, can't create PriceService`)
    }
